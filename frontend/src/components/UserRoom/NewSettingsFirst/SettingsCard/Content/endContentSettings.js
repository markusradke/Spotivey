...
export function EndContentSettings({
    ...
    const trimmed_end_url = String(endUrl ?? "").trim();
    ...
    const [rawFollowUps, setRawFollowUps] = React.useState(() => ({
        ...
        collectEmails: String(collectEmails ?? 0),
        emailTextEn: String(emailTextEn ?? ""),
        emailTextDe: String(emailTextDe ?? ""),
    }));
    ...
    return (
        <React.Fragment>
            ...
            {endOption !== "end_url" ? (
                <div
                    className="collect-emails-container"
                ...
                >
                    ...
                    />
                    <span>Collect participant emails for non-redirected participants, e.g., for a lottery</span>
                </div>
            ) : null}
            {collectEmails && (
                <div style={{ marginTop: '1rem' }}>
                    <TextField
                        label="Email Text (English)"
                        value={emailTextEn}
                        onChange={(e) => setEmailTextEn(e.target.value)}
                        fullWidth
                        multiline
                    />
                    <TextField
                        label="Email Text (German)"
                        value={emailTextDe}
                        onChange={(e) => setEmailTextDe(e.target.value)}
                        fullWidth
                        multiline
                        style={{ marginTop: '0.75rem' }}
                    />
