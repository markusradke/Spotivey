            ...
            if len(searchSpotifyList) > 0:
                for trackListResult in list(searchSpotifyList):
                    participant_list.append(
                        Participant.objects.get(id=trackListResult[1])
                    )
                    participant_id_list.append(
                        Participant.objects.filter(id=trackListResult[1]).values_list(
                            "participant"
                        )[0][0]
                    )
...
class GetRecentlyPlayedTracksSpotify(APIView):
    ...
    def post(self, request, format=None):
        ...
        for j in range(anzahl_tracks):
            ...
            context_type = (
                item_top_j.get("context").get("type")
                if item_top_j.get("context")
                else None
            )
            context_uri = (
                item_top_j.get("context").get("uri")
                if item_top_j.get("context")
                else None
            )
